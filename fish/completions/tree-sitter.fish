...
complete -c tree-sitter -n "__fish_tree_sitter_using_subcommand generate" -l stage -d 'Which generation stage to end after' -r -f -a "json\t'Generate `grammar.json` and `node-types.json`'
parser\t'Generate `parser.c` and related files'
lib\t'Compile to a library'"
complete -c tree-sitter -n "__fish_tree_sitter_using_subcommand generate" -l libdir -d 'The path to the directory containing the parser library' -r -F
...
complete -c tree-sitter -n "__fish_tree_sitter_using_subcommand generate" -s b -l build -d 'Deprecated: use --stage=lib'
...
complete -c tree-sitter -n "__fish_tree_sitter_using_subcommand generate" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c tree-sitter -n "__fish_tree_sitter_using_subcommand build" -s d -l docker -d 'No longer used'
